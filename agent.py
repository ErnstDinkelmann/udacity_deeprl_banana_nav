...
class Agent:
    ...
    def learn(self, experiences, gamma):
        ...
        with torch.no_grad():
            Q_targets = self.qnetwork_target(next_states).max(dim=1, keepdim=False)[0].to(device)
            Q_targets[dones.squeeze() == 1] = 0
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        e = self.experience(state, action, reward, next_state, done)  # create an instance of the experience tuple, which is a named tuple with 5 named properties
